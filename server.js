...
async function createFinbyIntent(req, res, fixedPlan = null) {
  try {
    ...
    // SAFE CORRECTION: Look for the form email if the user isn't logged in yet
    ...
    // SAFE CORRECTION: Pack structural keys so it matches what sth-fi-uk.html expects
    if (data.secrets) {
        data.paymentSecrets = data.secrets;
    }
...
// SAFE CORRECTION: Removed 'authenticateToken' security gate so public visitors can use checkout forms
...
export const biblicalProfiles = [
	...
	{ id: Mary = 4, name: "Mary", image: "/img/mary.jpg", description: "Mother of Jesus, blessed among women. Speak humbly, with grace and maternal love. Reference the joy and challenges of motherhood and faith." },
...
app.use((req, res, next) => {
	...
	res.send = function (data) {
		if (typeof data === "string" && data.includes("</body>")) {
			data = data.replace(
				"</body>",
				`<footer style="margin-top:40px;padding:20px;text-align:center;font-size:14px;color:#aaa;border-top:1px solid rgba(0,0,0,0.1);"><a href="/privacy-policy.html">Privacy Policy</a> | <a href="/terms-and-conditions.html">Terms & Conditions</a></footer></body>`
			);
		}
...
app.get("/api/chat/history", async (req, res) => {
	try {
		...
		const history = await pool.query("SELECT * FROM messages WHERE user_id = $1 AND character_id = $2 ORDER BY created_at ASC LIMIT 50", [userId, characterId]);
...
app.post("/api/chat", authenticateToken, async (req, res) => {
	try {
		...
		const userResult = await pool.query("SELECT plan, lifetime, expires_at, messages_sent FROM users WHERE id = $1", [userId]);
		...
		if (!isPaid && parseInt(userData.messages_sent) >= 3) {
			return res.status(403).json({ error: "LIMIT_REACHED", message: "You have used your 3 free divine consultations. Please choose an offering to continue." });
		}
		if (isPaid && !canAccessCharacter(userData, Number(characterId))) {
			return res.status(403).json({ error: "NO_ACCESS", message: "You do not have access to this character." });
		}

		await pool.query(`INSERT INTO messages (user_id, character_id, from_user, text) VALUES ($1, $2, true, $3)`, [userId, characterId, message]);

		const history = await pool.query(`SELECT * FROM messages WHERE user_id = $1 AND character_id = $2 ORDER BY created_at ASC LIMIT 20`, [userId, characterId]);
		const chatHistory = history.rows.map(m => ({ role: m.from_user ? "user" : "assistant", content: m.text }));

		const systemPrompt = `You are ${character.name}, a biblical figure.\n\n${character.description}\n\nRULES:\n- Speak in a biblical tone.\n- Do NOT say you are an AI.\n- Do NOT mention modern technology.\n- Stay fully in character as ${character.name}.\n- Speak with wisdom, authority, or humility appropriate to this figure.\n- Give spiritual and reflective answers.\n\nRemain in character at all times.`;
		...
		if (reply) {
			await pool.query(`INSERT INTO messages (user_id, character_id, from_user, text) VALUES ($1, $2, false, $3)`, [userId, characterId, reply]);
		}
		if (!isPaid) {
...
app.post("/finby-webhook", async (req, res) => {
  try {
    ...
    const status = data?.PaymentInformation?.Status || data?.paymentInformation?.status;
    const email = data?.PaymentInformation?.Debtor?.Email || data?.paymentInformation?.debtor?.email;
    const reference = data?.PaymentInformation?.References?.MerchantReference || data?.paymentInformation?.references?.merchantReference;

    if (!email) return res.status(400).json({ error: "Missing email" });

    let plan = "2995";
    let amount = 29.95;
    if (reference?.includes("3595")) { plan = "3595"; amount = 35.95; }
    if (reference?.includes("4995")) { plan = "4995"; amount = 49.95; }
    ...
    if (status === "Paid" || status === "Authorized") {
      ...
      if (plan === "4995") isLifetime = true;

      await pool.query("UPDATE users SET plan = $1, expires_at = $2, lifetime = $3, messages_sent = 0 WHERE email = $4", [plan, expiresAt, isLifetime, email]);
      const receiptPdf = makeReceiptPdfBase64({ email, plan, amount });

      await sendEmail(
        email,
        "Your Speak to Heaven receipt",
        "<h2>Payment received</h2><p>Thank you for your offering.</p><p><strong>Plan:</strong> " + plan + "</p><p><strong>Amount:</strong> £" + Number(amount).toFixed(2) + "</p>",
...
app.get("/test-receipt-email", async (req, res) => {
  ...
  const receiptPdf = makeReceiptPdfBase64({ email, plan: "lifetime", amount: 49.95 });

  await sendEmail(
    email,
    "TEST Receipt",
    "<h2>Thank you for your order with SpeakToHeaven.com</h2><p>We have received your payment successfully.</p><p>Your receipt is attached to this email as a PDF.</p><p><strong>Plan:</strong> Lifetime Access</p><p><strong>Amount paid:</strong> £49.95</p>",
